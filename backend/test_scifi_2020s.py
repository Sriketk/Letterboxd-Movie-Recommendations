...
async def test_scifi_2020s():
    print("🚀 Testing Sci-Fi Movies from 2020s...")
    print("📋 Request parameters:")
    print("   Username: sriketk")
    print("   Genres: science_fiction")
    ...
    try:
        recommendations = await recommend_n_movies(
            num_recs=25,  # Get more recommendations
            user="sriketk",
        ...
        print(
            f"📊 Total sci-fi movies found: {len(recommendations['recommendations'])}"
        )
        print()
        print("🎯 Top sci-fi recommendations:")

        for i, movie in enumerate(
            recommendations["recommendations"].head(25).itertuples(), 1
        ):
            print(
